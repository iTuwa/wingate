...
def school_attendance_app(title, database_file, attendance_log_file):
    ...
    if selected == "Attendance":
        ...
        if st.button("Sign In"):
            if df["PIN_Dad"].isin([child_pin]).any() or df["PIN_Mom"].isin([child_pin]).any():
                student_data = df[(df["PIN_Dad"] == child_pin) | (df["PIN_Mom"] == child_pin)]
                child_name = student_data.iloc[0]["Name"]
                dad_email = student_data.iloc[0]["Dad Email"]
                mom_email = student_data.iloc[0]["Mom Email"]
                last_action = student_data.iloc[0]["Last Action"]
                if last_action.startswith("Sign In"):
                    st.error(f"{child_name} is already signed in.")
                else:
        ...
        if st.button("Sign Out"):
            if df["PIN_Dad"].isin([child_pin]).any() or df["PIN_Mom"].isin([child_pin]).any():
                ...
                mom_email = student_data.iloc[0]["Mom Email"]
                if last_action.startswith("Sign In"):
                    if send_email(dad_email, child_name, "signed out", "dad") or send_email(mom_email, child_name, "signed out", "mom"):
                        st.success(f"{child_name} signed out successfully!")
                        log_attendance(child_name, "parent", "sign out", attendance_log_file)
                        df.loc[df["Name"] == child_name, "Last Action"] = "Sign Out"
                        df.to_excel(database_file, index=False)
                        st.session_state.children_database = df
                else:
                    st.error(f"{child_name} must sign in first before signing out.")
            else:
                st.error("Invalid PIN. Please enter a valid PIN.")
