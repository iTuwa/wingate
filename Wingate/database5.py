    ...
    if selected == "Attendance":
        ...
        if st.button("Sign Out"):
            if df["PIN_Dad"].isin([child_pin]).any() or df["PIN_Mom"].isin([child_pin]).any():
                ...
                mom_email = student_data.iloc[0]["Mom Email"]
                if last_action.startswith("Sign In"):
                    if df["PIN_Dad"].isin([child_pin]).any() and send_email(dad_email, child_name, "signed out", "dad"):
                        st.success(f"{child_name} signed out successfully by dad!")
                        log_attendance(child_name, "dad", "sign out", attendance_log_file)
                    elif df["PIN_Mom"].isin([child_pin]).any() and send_email(mom_email, child_name, "signed out", "mom"):
